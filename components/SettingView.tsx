    ...
    return (
      <div className="space-y-6 animate-fade-in pb-12">
        ...
        <div className="flex gap-3 pt-4">
            ...
            <button onClick={handleSaveProfile} className="flex-1 py-4 bg-st-blue text-white font-bold rounded-2xl disabled:opacity-50" disabled={isSaving}>{isSaving ? '저장 중...' : '저장'}</button>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm divide-y divide-st-box/50 overflow-hidden">
                    ...
                    {(user.interests.business.length > 0 || user.interests.lifestyle.length > 0) && (
                        <div className="flex flex-wrap gap-1 mt-1">
                            {user.interests.business.slice(0, 2).map(i => (
                                <span key={i} className="px-1.5 py-0.5 bg-st-bg text-st-muted text-[10px] font-medium rounded">{i}</span>
                            ))}
        ...
        <div className="p-6 space-y-4">
            ...
            {syncError && (
                <div className="p-4 bg-st-red/10 rounded-2xl animate-shake">
            ...
            <div className="flex items-center justify-between p-4 bg-st-bg rounded-2xl">
                ...
                <button
                    onClick={handleSync}
                    className="px-4 py-1.5 rounded-full text-xs font-bold transition-all bg-st-blue text-white shadow-sm hover:bg-st-blue/80"
                >
                    연동하기
                </button>
            </div>
